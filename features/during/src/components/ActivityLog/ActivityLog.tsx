...
export const ActivityLog = ({ activities, properties }: UpstreamMyJourneyActivities[]) => {
  ...
  useEffect(() => {
    const sorted = [...activities].sort(
      (a, b) => new Date(b.dateOfActivity).getTime() - new Date(a.dateOfActivity).getTime(),
    )
  ...
  const onRecentClick = () => {
    if (showOldest) {
      const sorted = [...sortedActivities].sort(
        (a, b) => new Date(b.dateOfActivity).getTime() - new Date(a.dateOfActivity).getTime(),
      )
      setSortedActivities(sorted)
      setShowOldest(false)
  ...
  const onOldestClick = () => {
    if (!showOldest) {
      const sorted = [...sortedActivities].sort(
        (a, b) => new Date(a.dateOfActivity).getTime() - new Date(b.dateOfActivity).getTime(),
      )
      setSortedActivities(sorted)
      setShowOldest(true)
      setPage(1) // Reset to the first page
